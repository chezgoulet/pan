...
fn every_fixture_round_trips_through_pan() {
    let fixtures = load();
    assert_eq!(fixtures.len(), 15, "expected 15 fixtures, found {}", fixtures.len());
    for fx in &fixtures {
        let s = fx.envelope.to_ndjson()
            .unwrap_or_else(|e| panic!("{}: serialize: {e}", fx.name));
        let back: pan_daemon::wire::Envelope = serde_json::from_str(&s)
            .unwrap_or_else(|e| panic!("{}: re-parse: {e}", fx.name));
...
fn every_fixture_decodes_into_its_typed_body() {
    ...
    for fx in &fixtures {
        match &fx.envelope.body {
            Body::Hello(_) | Body::Welcome(_)
            | Body::RegisterCapabilities(_) | Body::InstantiateSoul(_)
            | Body::ReleaseSoul(_) | Body::Perceive(_)
            | Body::Decision(_) | Body::Ack(_)
            | Body::Error(_) | Body::Shutdown(_) => {
...
fn fixtures_cover_all_ten_message_types() {
    let report = check_fixtures(&fixtures_dir())
        .expect("conformance loader failed");
    if !report.is_ok() {
        for e in &report.errors { eprintln!("conformance: {e}"); }
        panic!("conformance reported {} problem(s); see stderr", report.errors.len());
    }
    assert_eq!(report.type_count, 10,
        "all 10 message types should be covered; only {} are", report.type_count);
    assert_eq!(report.fixture_count, 15,
        "expected 15 fixtures; got {}", report.fixture_count);
}
...
fn every_fixture_survives_compact_ndjson_round_trip() {
    for fx in load_fixtures(&fixtures_dir()).unwrap() {
        ...
        assert_eq!(reparsed, original, "{}: ndjson drifted from original", fx.name);
    }
}
...
#[test] fn fixture_01_hello_round_trips() { assert_round_trip("01_hello"); }
#[test] fn fixture_02_welcome_round_trips() { assert_round_trip("02_welcome"); }
#[test] fn fixture_03_register_capabilities_round_trips() { assert_round_trip("03_register_capabilities"); }
#[test] fn fixture_04_instantiate_soul_round_trips() { assert_round_trip("04_instantiate_soul"); }
#[test] fn fixture_05_perceive_utterance_round_trips() { assert_round_trip("05_perceive_utterance"); }
#[test] fn fixture_06_decision_express_round_trips() { assert_round_trip("06_decision_express"); }
#[test] fn fixture_07_perceive_event_round_trips() { assert_round_trip("07_perceive_event"); }
#[test] fn fixture_08_decision_invoke_move_round_trips() { assert_round_trip("08_decision_invoke_move"); }
#[test] fn fixture_09_error_unknown_capability_round_trips() { assert_round_trip("09_error_unknown_capability"); }
#[test] fn fixture_10_perceive_superseding_revision_round_trips() { assert_round_trip("10_perceive_superseding_revision"); }
#[test] fn fixture_11_perceive_tick_round_trips() { assert_round_trip("11_perceive_tick"); }
#[test] fn fixture_12_perceive_signal_round_trips() { assert_round_trip("12_perceive_signal"); }
#[test] fn fixture_13_ack_round_trips() { assert_round_trip("13_ack"); }
#[test] fn fixture_14_shutdown_round_trips() { assert_round_trip("14_shutdown"); }
#[test] fn fixture_15_release_soul_round_trips() { assert_round_trip("15_release_soul"); }

fn assert_round_trip(stem: &str) {
    let target = format!("{stem}.json");
    let fx = load().into_iter()
        .find(|f| f.name == target)
        .unwrap_or_else(|| panic!("fixture {target} not found"));
    let s = fx.envelope.to_ndjson()
        .unwrap_or_else(|e| panic!("{target}: serialize: {e}"));
    let back: pan_daemon::wire::Envelope = serde_json::from_str(&s)
        .unwrap_or_else(|e| panic!("{target}: re-parse: {e}"));
