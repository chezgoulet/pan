...
- [x] Plugin lifecycle: `Register → Provision → Validate → Run → Cleanup` (Caddy-style),
      hierarchical IDs, explicit conflict = provision-time error (never last-wins).
- [x] The capability-handle wiring registry: trait-object handles granted at provision.
...
- [x] The abandon-path: cleanly discard an in-flight `Decision` whose goal was superseded.
...
> **DONE.** Implemented in `pan-core/` as a compiling, tested crate. Exit test is
> `tests::wave0_exit_test` (passing). The non-bypassable pipeline is enforced at the
> type level and the bypass is proven to fail compilation (`tests/compile-fail/`).
> Run `./verify.sh`.
